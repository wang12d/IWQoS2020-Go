...
func main() {
	...
	requesterRegistrationTimeCost, workerRegistraionTimeCost := make([]time.Duration, length), make([]time.Duration, length)
	...
	var requesterRegistraionCost, workerRegistrationCost time.Duration
	...
	for i := 0; i < int(numberOfIteration); i++ {
		for n, workerRequired := range numberOfWorkers {
			...
			requesterRegistraionCost, workerRegistrationCost = 0, 0
			communicationCost = 0
			r := requester.NewRequester(byteSize)
			timeStart = time.Now()
			r.Register()
			requesterRegistraionCost += time.Since(timeStart)
			registrationCost += requesterRegistraionCost
			requesterRegistrationTimeCost[n] += requesterRegistraionCost
			...
			workerRegistraionTimeCost[n] += workerRegistrationCost / time.Duration(workerRequired)
	...
	fmt.Printf("PFCrowd: %v\n", byteSize)
	for i := 0; i < length; i++ {
		fmt.Printf("%v(%v, %v),%v,%v,%v,%v,%v,%v\n", registrationTimeCost[i]/time.Duration(numberOfIteration),
			requesterRegistrationTimeCost[i]/time.Duration(numberOfIteration), workerRegistraionTimeCost[i]/time.Duration(numberOfIteration),
